...
Razorpay\'s API is used in **four places** across the system --- invoice
polling, payment link generation, and two webhook receivers. Here\'s the
complete picture:
...
::: card
::: card-header
[2. Recovery Router + Escalation Agent --- POST
/v1/payment_links/]{.card-title} [PAYMENT LINKS]{.tag .tag--green}
:::

**Where:** Workflow 1 → \"Generate Payment Link\" node + Workflow 5 →
\"Generate Esc Payment Link\" node
...
recovery attempt. The link includes customer details, amount (in paise),
reference_id matching the original order, and metadata (source, event
type, failure category, recovery event ID). The `short_url` from the
response is injected into WhatsApp/Email/SMS messages, replacing
placeholder links.

::: {.node-detail style="margin-top:1rem;"}
::: node-detail-title
...
Returns: `{ id, short_url, status, ... }` --- the `short_url` is the
...
::: card
::: card-header
[3. Recovery Tracker --- Webhook Receiver]{.card-title} [WEBHOOK]{.tag
.tag--green}
:::
...
this URL and select both `payment.captured` and `payment_link.paid`
events. This hasn\'t been configured yet, which is why the tracker
hasn\'t fired from live events.
...
**Autonomous escalation agent** --- every 5 minutes, AI reviews due
cases in micro-batches of 20, decides the next action (channel switch,
tone escalation, or give up), sends messages, logs attempts, and updates
state --- a true observe-reason-act loop
:::
...
**Unrecoverable decline detection** --- fraud/stolen card events and
international card blocks are classified with probability 0,
automatically marked as exhausted with no contact attempted
:::
...
**Razorpay Payment Links** --- generates unique one-tap payment links
via Razorpay API for every recovery message, replacing placeholder URLs
with real payment links tied to the original order
:::
...
::: table-wrap
  ...
  Razorpay API                   Invoice fetching (GET /v1/invoices), Payment Links (POST /v1/payment_links/), and payment webhook receiver
...
::: table-wrap
  ...
  Razor router       httpBasicAuth   `frkgneFwFDPU9g7U`   Invoice Scanner, Generate Payment Link (WF1), Generate Esc Payment Link (WF5)
...
[→]{.flow-arrow} [Generate Payment Link]{.flow-node} [→]{.flow-arrow}
[Route Switch]{.flow-node} [→]{.flow-arrow} [Send + Log
Attempt]{.flow-node}
:::
:::
...
Detects incoming format: if the payload is a raw Razorpay
`payment.failed` webhook (`body.event === 'payment.failed'`), it
extracts from `payload.payment.entity.*` including `error_description`,
`error_reason`, `error_source`, `error_step`. Otherwise handles the
custom Recovery Router format. Normalizes 15 fields with null-safe
defaults. Sets `razorpay_raw: true/false` flag.
:::
...
**Type:** n8n-nodes-base.httpRequest (v4.4)   **Auth:** httpBasicAuth
(\"Razor router\")   **Error handling:** continueRegularOutput

**URL:** `POST https://api.razorpay.com/v1/payment_links/`

Creates a unique Razorpay payment link for this recovery attempt.
Payload includes: amount (in paise), currency, customer details,
`reference_id` matching the original `order_id`, and notes with
source/event_type/failure_category/recovery_event_id. Returns
`short_url` --- the one-tap payment link injected into all downstream
messages.
:::
...
Receives Razorpay `payment.captured` and `payment_link.paid` webhook
events. Both must be configured in Razorpay Dashboard → Webhooks.
:::
...
Handles both `payment.captured` and `payment_link.paid` events. For
payment_link.paid: extracts from both `payload.payment_link.entity`
(payment_link_id, reference_id) and `payload.payment.entity`
(payment_id, amount, method). For payment.captured: extracts payment_id,
amount, order_id, method from the payment entity.
:::
...
`order_id = $json.order_id AND status = 'pending'`. Matches by order_id
(not payment_id) because retry payments create new payment IDs ---
order_id links the recovery to the original failure. Limit 1.
:::
...
`payment_id=new_payment_id` (from successful retry)
...
[ACTIVE]{.tag .tag--green} [14 NODES]{.tag}
:::
...
**Purpose:** The TRUE autonomous agent. Every 5 minutes: fetches up to
20 events where `next_action_at <= now()`, reasons about the optimal
next action for each, generates a unique payment link, acts (sends
messages via the right channel), logs every attempt, updates state, and
stops when recovery window expires or max attempts reached. Micro-batch
architecture avoids the scaling bottleneck of bulk-processing all
pending events.

::: flow
[Schedule (5min)]{.flow-node} [→]{.flow-arrow} [Fetch Due Events
(≤20)]{.flow-node} [→]{.flow-arrow} [Filter Actionable]{.flow-node}
[→]{.flow-arrow} [Generate Esc Payment Link]{.flow-node}
[→]{.flow-arrow} [AI Escalation Decision]{.flow-node} [→]{.flow-arrow}
[Route]{.flow-node} [→]{.flow-arrow} [Send]{.flow-node} [→]{.flow-arrow}
[Log Attempt]{.flow-node} [→]{.flow-arrow} [Update State]{.flow-node}
:::
:::
...
`status = 'pending' AND next_action_at <= now()`, limit 20

Pre-filters at the database level --- only fetches events that are
actually due for action. Combined with the 5-minute schedule, this
creates a responsive micro-batch system that processes events as they
become due instead of bulk-loading everything.
:::
...
Secondary filter: removes events that are opted out, past recovery
window, at max attempts, or classified as
`unrecoverable_decline`/`fraud_detected`/zero probability. Returns empty
array marker `[{ json: { _empty: true } }]` when no actionable items
remain.
:::
...
Creates a fresh payment link for each escalation attempt, with updated
amount and reference_id. The `short_url` is passed to the AI Escalation
Decision and injected into all outbound messages.
:::
...
pending, exhausted counts, recovery rate %, amounts,
avg_recovery_time_hours), `by_event_type` (with recovery_rate per type),
`by_channel` (with recovery_rate per channel), `by_failure_category`
(all categories with avg probability and recovery_rate),
`channel_ranking` (channels sorted by recovery rate), and `ai_lift` ---
baseline comparison metrics showing AI recovery rate vs. Razorpay\'s
15-20% simple retry baseline (17.5% midpoint), with improvement_points,
lift_multiplier, and additional_revenue_recovered.
:::
...
::: card
::: card-header
[Payment Link Nodes Need Manual Credential]{.card-title} [ONE-TIME
SETUP]{.tag .tag--amber}
:::

The \"Generate Payment Link\" node (Recovery Router, WF1) and \"Generate
Esc Payment Link\" node (Escalation Agent, WF5) were added
programmatically but n8n\'s API cannot assign httpBasicAuth credentials
to httpRequest nodes. **One-click fix:** Open each node in the n8n
editor → Credential for HTTP Basic Auth → Select \"Razor router\" →
Save. Until this is done, the payment link generation will return an
error (the workflows continue via onError:continueRegularOutput, so
messages still send --- just with fallback URLs instead of real payment
links).
:::
:::
:::
